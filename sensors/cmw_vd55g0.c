...
static int32_t CMW_VD55G0_GetSensorInfo(void *io_ctx, ISP_SensorInfoTypeDef *info)
{
  ...
  /* Get sensor name */
  if (sizeof(info->name) >= strlen(VD55G0_NAME) + 1)
  {
  ...
  switch (((CMW_VD55G0_t *)io_ctx)->ctx_driver.bayer)
  {
    case VD55G0_BAYER_NONE:
      info->bayer_pattern = ISP_DEMOS_TYPE_MONO;
      break;
    case VD55G0_BAYER_RGGB:
      info->bayer_pattern = ISP_DEMOS_TYPE_RGGB;
      break;
    case VD55G0_BAYER_GRBG:
      info->bayer_pattern = ISP_DEMOS_TYPE_GRBG;
      break;
    case VD55G0_BAYER_GBRG:
      info->bayer_pattern = ISP_DEMOS_TYPE_GBRG;
      break;
    case VD55G0_BAYER_BGGR:
  ...
  info->color_depth = ((CMW_VD55G0_t *)io_ctx)->ctx_driver.ctx.config_save.pixel_depth;
  ...
  again_min_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(32.0 / (32.0 - (double)VD55G0_ANALOG_GAIN_MIN)) + 0.5);
  again_max_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(32.0 / (32.0 - (double)VD55G0_ANALOG_GAIN_MAX)) + 0.5);
  dgain_min_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(FP58_TO_FLOAT(VD55G0_DIGITAL_GAIN_MIN)) + 0.5);
  dgain_max_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(FP58_TO_FLOAT(VD55G0_DIGITAL_GAIN_MAX)) + 0.5);

  info->gain_min = again_min_mdB + dgain_min_mdB;
  info->gain_max = again_max_mdB + dgain_max_mdB;
  info->again_max = again_max_mdB;

  ret = VD55G0_GetExposureRegRange(&((CMW_VD55G0_t *)io_ctx)->ctx_driver, &exposure_min, &exposure_max);
  if (ret)
    return ret;
...
static int CMW_VD55G0_GetResType(uint32_t width, uint32_t height, VD55G0_Res_t *res)
{
  ...
  else if (width == 640 && height == 600)
  {
      *res = VD55G0_RES_FULL_640_600;
  }
  else
  {
    return CMW_ERROR_WRONG_PARAM;
...
static int32_t CMW_VD55G0_Init(void *io_ctx, CMW_Sensor_Init_t *initSensor)
{
  ...
  config.ext_clock_freq_in_hz = CAMERA_VD55G0_FREQ_IN_HZ;
  ...
  config.flicker = VD55G0_FLICKER_FREE_NONE;
  ...
  if (((CMW_VD55G0_t *)io_ctx)->ctx_driver.bayer == VD55G0_BAYER_NONE)
  {
...
int32_t CMW_VD55G0_SetGain(void *io_ctx, int32_t gain)
{
  CMW_VD55G0_t *ctx = (CMW_VD55G0_t *)io_ctx;
  uint32_t again_min_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(32.0 / (32.0 - (double)VD55G0_ANALOG_GAIN_MIN)) + 0.5);
  uint32_t again_max_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(32.0 / (32.0 - (double)VD55G0_ANALOG_GAIN_MAX)) + 0.5);
  uint32_t dgain_min_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(FP58_TO_FLOAT(VD55G0_DIGITAL_GAIN_MIN)) + 0.5);
  uint32_t dgain_max_mdB = (uint32_t)(LINEAR_TO_MDECIBEL(FP58_TO_FLOAT(VD55G0_DIGITAL_GAIN_MAX)) + 0.5);
  ...
  ret = VD55G0_SetAnalogGain(&ctx->ctx_driver, analog_reg);
  if (ret)
    return ret;
  ...
  ret = VD55G0_SetDigitalGain(&ctx->ctx_driver, (int)digital_reg);
  if (ret)
    return ret;
...
int32_t CMW_VD55G0_SetExposure(void *io_ctx, int32_t exposure)
{
  return VD55G0_SetExposureTime(&((CMW_VD55G0_t *)io_ctx)->ctx_driver, exposure);
}

int32_t CMW_VD55G0_SetExposureMode(void *io_ctx, int32_t mode)
{
  int ret = -1;
  ...
  switch (mode)
  {
    case CMW_EXPOSUREMODE_MANUAL:
      ret = VD55G0_SetExposureMode(&((CMW_VD55G0_t *)io_ctx)->ctx_driver, VD55G0_EXPOSURE_MODE_MANUAL);
      break;
    case CMW_EXPOSUREMODE_AUTOFREEZE:
      ret = VD55G0_SetExposureMode(&((CMW_VD55G0_t *)io_ctx)->ctx_driver, VD55G0_EXPOSURE_MODE_FREEZE);
      break;
    case CMW_EXPOSUREMODE_AUTO:
    default:
      ret = VD55G0_SetExposureMode(&((CMW_VD55G0_t *)io_ctx)->ctx_driver, VD55G0_EXPOSURE_MODE_AUTO);
      break;
  }
